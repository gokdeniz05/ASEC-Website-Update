...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || !validateCSRFToken($_POST['csrf_token'])) {
        ...
        if ($user_type === 'corporate') {
            ...
            if (!validateEmail($email)) { $error = 'Geçersiz e-posta adresi!'; }
            else if (!validatePhone($phone)) { $error = 'Geçersiz telefon numarası!'; }
            else if ($password !== $password2) { $error = 'Şifreler eşleşmiyor!'; }
            else {
                $password_check = validatePassword($password);
                if (!$password_check['valid']) { $error = $password_check['message']; }
                else if (!$company_name || !$contact_person || !$phone || !$email || !$password) { $error = 'Lütfen zorunlu alanları doldurun!'; }
                else {
                    $captcha_response = $_POST['g-recaptcha-response'] ?? '';
                    if (!validateCaptcha($captcha_response)) { $error = 'Robot olmadığınızı doğrulayın!'; }
                    else {
                        $stmt = $pdo->prepare('SELECT id FROM users WHERE email = ?');
                        $stmt->execute([$email]);
                        if ($stmt->fetch()) { $error = 'Bu e-posta adresi zaten bireysel kullanıcı olarak kayıtlı!'; }
                        else {
                             $stmt = $pdo->prepare('SELECT id FROM corporate_users WHERE email = ?');
                             $stmt->execute([$email]);
                             if ($stmt->fetch()) { $error = 'Bu e-posta adresi zaten kurumsal kullanıcı olarak kayıtlı!'; }
                             else {
                                 $stmt = $pdo->prepare('SELECT id FROM corporate_requests WHERE email = ? AND status IN ("pending", "approved")');
                                 $stmt->execute([$email]);
                                 if ($stmt->fetch()) { $error = 'Bu e-posta adresi için bekleyen istek var!'; }
                                 else {
                                    ...
                                    if ($ok) {
                                        $success = true;
                                        $activeTab = 'corporate';
                                        $success_message = __t('register.corporate.success');
                                    } else {
        ...
        // BİREYSEL KAYIT İŞLEMLERİ
            ...
            if (!validateEmail($email)) { $error = 'Geçersiz e-posta adresi!'; }
                ...
                $password_check = validatePassword($password);
                if (!$password_check['valid']) { $error = $password_check['message']; }
                else if (!$name || !$phone || !$email || !$university || !$department || !$class || !$password) { $error = 'Tüm alanları doldurun!'; }
                else {
                    $captcha_response = $_POST['g-recaptcha-response'] ?? '';
                    if (!validateCaptcha($captcha_response)) { $error = 'Robot olmadığınızı doğrulayın!'; }
                    else {
                        $stmt = $pdo->prepare('SELECT id FROM users WHERE email = ?');
                        $stmt->execute([$email]);
                        if ($stmt->fetch()) { $error = 'Bu e-posta adresi zaten kayıtlı!'; }
                        else {
                            $hashed = password_hash($password, PASSWORD_DEFAULT);
...
<body>
    ...
    <main class="auth-page">
        <div class="auth-container">
            <div class="auth-tabs">
                <button type="button" class="auth-tab <?php echo $activeTab === 'individual' ? 'active' : ''; ?>" onclick="switchTab('individual')">
                    <?php echo __t('register.tab.individual'); ?>
                </button>
                <button type="button" class="auth-tab corporate <?php echo $activeTab === 'corporate' ? 'active' : ''; ?>" onclick="switchTab('corporate')">
                    <?php echo __t('register.tab.corporate'); ?>
                </button>
            </div>
            
            <div id="individual-form" class="auth-form-container <?php echo $activeTab === 'individual' ? 'active' : ''; ?>">
                <h2><?php echo __t('register.tab.individual'); ?></h2>
                <form method="post">
            ...
            <div id="corporate-form" class="auth-form-container <?php echo $activeTab === 'corporate' ? 'active' : ''; ?>">
                <h2><?php echo __t('register.tab.corporate'); ?></h2>
                <form method="post">
                    ...
                    <div class="form-group">
                        <label for="corporate_password"><?php echo __t('register.password'); ?> <span style="color: red;">*</span></label>
                        <input type="password" id="corporate_password" name="password" required>
                    </div>
                    <div class="form-group">
                        <label for="corporate_password2"><?php echo __t('register.password2'); ?> <span style="color: red;">*</span></label>
                ...
                <p><?php echo __t('register.have_account'); ?> <a href="login.php?tab=corporate"><?php echo __t('register.login'); ?></a></p>
            </div>
        </div>
    </main>
